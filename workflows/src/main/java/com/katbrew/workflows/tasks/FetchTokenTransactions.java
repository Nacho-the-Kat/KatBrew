...
//                            .get()
//                            .uri(tokenUrl + "/token/operations?tick=" + token.getTick() + (cursor.compareTo(new BigInteger("0")) > 0 ? "&next=" + cursor : ""))
//                            .retrieve()
...
//            }
//
//            final List<Transaction> dbEntries = transactionService.findBy(List.of(Tables.TRANSACTION.FK_TOKEN.eq(token.getId())));
//
...
//            transactionService.batchInsertVoid(transactionList.stream().filter(single -> single.getId() == null).toList());
//        });
